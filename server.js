...
app.post('/api/create', function (req, res) {
    ...
    if (req.session.auth) {
        MongoClient.connect(mongourl, function (err, db) {
            ...
            createRestaurantApi(db, req.files, req.body, userid,
                    function (result) {
                        db.close();
...
function createRestaurantApi(db, files, query, userid, callback) {

    var data = "";
    var mimetype = "";
    if (files) {
        data = new Buffer(files.photo.data)
                .toString('base64');
